/** @file GenrootRawEvent.h
    @brief Defines the GenrootRawEvent class to read ART data into genroot
    @author bloer
    @ingroup daqman
*/

#ifndef GenrootRawEvent_INCLUDED
#define GenrootRawEvent_INCLUDED

#include <stdlib.h>
#include <stdint.h>
#include <vector>

/** @class GenrootRawEvent
...
public:
  ...
  GenrootRawEvent(uint32_t event_id, uint32_t timestamp, uint32_t run_id);
  ///Destructor 
  ~GenrootRawEvent();
...
public:
  ...
  static long GetTotalBufferSize(){ return _total_buffer_size;}
...
private:
  ...
  GenrootRawEvent(const GenrootRawEvent& right);
  GenrootRawEvent& operator=(const GenrootRawEvent& right);
};
